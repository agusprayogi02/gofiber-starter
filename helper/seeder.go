...
func RunAllSeeders(db *gorm.DB) error {
	...
	for _, seeder := range seeders {
		Info(fmt.Sprintf("Running seeder: %s", seeder.Name))
...
func SeedUsers(db *gorm.DB) error {
	...
	for _, user := range users {
		...
		if result.Error == gorm.ErrRecordNotFound {
			...
			Info(fmt.Sprintf("Created user: %s", user.Email))
		} else {
			Info(fmt.Sprintf("User already exists: %s", user.Email))
		}
	}
...
func SeedPosts(db *gorm.DB) error {
	...
	for _, post := range posts {
		...
		if result.Error == gorm.ErrRecordNotFound {
			...
			Info(fmt.Sprintf("Created post: %s", truncateText(post.Tweet, 50)))
		} else {
			Info(fmt.Sprintf("Post already exists: %s", truncateText(post.Tweet, 50)))
		}
	}
...
func ResetDatabase(db *gorm.DB) error {
	...
	for _, table := range tables {
		if err := TruncateTable(db, table); err != nil {
			Warn(fmt.Sprintf("Failed to truncate table %s: %v", table, err))
		}
	}
