...
func RegisterPeriodicTasks(scheduler *asynq.Scheduler) error {
	...
	_, err := scheduler.Register(
		"@daily 2:00",
		asynq.NewTask(helper.TaskCleanupOldFiles, []byte(`{"directory":"public/uploads","older_than":30}`)),
		asynq.Queue(helper.QueueLow),
	)
	if err != nil {
	...
	_, err = scheduler.Register(
		"0 8 * * MON",
		asynq.NewTask(helper.TaskGenerateReport, []byte(`{"report_type":"weekly","filters":{}}`)),
		asynq.Queue(helper.QueueDefault),
	)
	if err != nil {
	...
	helper.Logger.Info("All periodic tasks registered successfully")
	return nil
}
...
func HandleHealthCheck(ctx context.Context, t *asynq.Task) error {
	helper.Logger.Info("Running scheduled health check")
	...
	helper.Logger.Info("Health check completed")
	return nil
}
...
func HandleCleanupExpiredTokens(ctx context.Context, t *asynq.Task) error {
	helper.Logger.Info("Running scheduled token cleanup")
...
func HandleMonthlyArchive(ctx context.Context, t *asynq.Task) error {
	...
	helper.Logger.Info("Monthly archive completed")
	return nil
}
...
func ScheduleDailyBackup(scheduler *asynq.Scheduler, hour, minute int) error {
	...
	helper.Logger.Info(fmt.Sprintf("Daily backup scheduled at %02d:%02d", hour, minute))
	return nil
}

// HandleDatabaseBackup handler for database backup
func HandleDatabaseBackup(ctx context.Context, t *asynq.Task) error {
	helper.Logger.Info("Starting database backup")
	...
	helper.Logger.Info("Database backup completed")
	return nil
}
...
func ScheduleEmailDigest(scheduler *asynq.Scheduler, hour, minute int, recipients []string) error {
	...
	_, err := scheduler.Register(
		cronSpec,
		asynq.NewTask("email:daily_digest", helper.ToJSON(payload)),
		asynq.Queue(helper.QueueDefault),
	)
	if err != nil {
...
func HandleDailyEmailDigest(ctx context.Context, t *asynq.Task) error {
	helper.Logger.Info("Sending daily email digest")
	...
	helper.Logger.Info("Daily email digest sent")
	return nil
}
...
func HandleMetricsCollection(ctx context.Context, t *asynq.Task) error {
	helper.Logger.Info("Collecting metrics")
	...
	helper.Logger.Info("Metrics collection completed")
	return nil
}
...
func ScheduleOneTimeTask(taskType string, payload interface{}, executeAt time.Time) error {
	return helper.EnqueueTaskAt(taskType, payload, executeAt)
}
...
func ScheduleRecurringTask(scheduler *asynq.Scheduler, taskType string, payload interface{}, interval time.Duration) error {
	...
	_, err := scheduler.Register(
		cronSpec,
		asynq.NewTask(taskType, helper.ToJSON(payload)),
	)
...
func ListScheduledTasks() []string {
	...
	helper.Logger.Info("Listing scheduled tasks not yet implemented")
	return []string{}
}
