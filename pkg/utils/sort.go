...
import (
	...
	"starter-gofiber/pkg/pagination"
...
func CombineSortAndPagination(db *gorm.DB, sortConfig SortConfig, paginate pagination.CursorPagination) *gorm.DB {
	// Apply sorting first
	db = ApplySortConfig(db, sortConfig)

	// Then apply cursor pagination
	db, _ = pagination.ApplyCursorPagination(db, paginate)
