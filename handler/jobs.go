...
func (h *JobHandler) TriggerEmailJob(c *fiber.Ctx) error {
	...
	if err := jobs.SendEmailJob(req.To, req.Subject, req.Body, nil); err != nil {
		helper.Logger.Error(fmt.Sprintf("Failed to enqueue email job: %v", err))
...
func (h *JobHandler) TriggerExportJob(c *fiber.Ctx) error {
	...
	// Dispatch export job
	if err := jobs.ProcessExportJob(userID, req.ExportType, req.Query, filename); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func (h *JobHandler) PauseQueue(c *fiber.Ctx) error {
	...
	if err := helper.PauseQueue(queueName); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func (h *JobHandler) ResumeQueue(c *fiber.Ctx) error {
	...
	if err := helper.UnpauseQueue(queueName); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func (h *JobHandler) RetryFailedJob(c *fiber.Ctx) error {
	...
	if err := helper.RetryTask(req.Queue, req.TaskID); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func (h *JobHandler) DeletePendingTasks(c *fiber.Ctx) error {
	...
	count, err := helper.DeleteAllPendingTasks(queueName)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
