...
func CacheMiddleware(cfg ...CacheConfig) fiber.Handler {
	...
	return func(c *fiber.Ctx) error {
		...
		cached, err := helper.RedisClient.Get(ctx, cacheKey).Bytes()
		...
		if shouldCache && status >= 200 && status < 300 {
			...
			if err := helper.RedisClient.Set(ctx, cacheKey, responseBody, cacheConfig.Expiration).Err(); err != nil {
...
func InvalidateCache(c *fiber.Ctx, paths ...string) error {
	...
	for _, path := range paths {
		...
		// Find matching keys
		keys, err := helper.RedisClient.Keys(ctx, pattern).Result()
		if err != nil {
			return err
		}
		...
		if len(keys) > 0 {
			if err := helper.RedisClient.Del(ctx, keys...).Err(); err != nil {
				return err
			}
		}
	}
...
func InvalidateCacheByPattern(pattern string) error {
	...
	if len(keys) > 0 {
		return helper.RedisClient.Del(ctx, keys...).Err()
	}
...
func ClearAllCache() error {
	...
	keys, err := helper.RedisClient.Keys(ctx, "cache:*").Result()
	if err != nil {
		return err
	}
